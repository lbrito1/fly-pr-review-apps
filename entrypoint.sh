...
setup() {
  ...
  if [ -n "$postgres_app" ]; then
    ...
    if ! flyctl status --app "$postgres_app"; then
      ...
      flyctl postgres create --name "$postgres_app" --region "$region" --org "$org" --fork-from "$postgres_source_app" --vm-size shared-cpu-4x --volume-size 20 --initial-cluster-size 1 || true
